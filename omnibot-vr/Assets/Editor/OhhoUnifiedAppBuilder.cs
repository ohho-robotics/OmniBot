...
namespace OmniBot.VR.Editor
{
    ...
    public static class OhhoUnifiedAppBuilder
    {
        ...
        public static void Build()
        {
            ...
            {
                Debug.LogError("[OhhO] You cannot rebuild the UI while in Play Mode! Please click the Play button to stop, then rebuild.");
                return;
            }
            FixHands();
            FixEventSystem();
            ...
            WireApp(screens.Item1, screens.Item2, screens.Item3, screens.Item4);
        ...
        private static void FixHands()
        {
            ...
            {
                bool changed = false;
                if (config.handTrackingSupport == OVRProjectConfig.HandTrackingSupport.ControllersOnly)
                {
                    config.handTrackingSupport = OVRProjectConfig.HandTrackingSupport.ControllersAndHands;
                    changed = true;
                }
                if (!config.requiresSystemKeyboard)
                {
        ...
        private static void DeleteOldUi()
        {
            ...
            foreach (var n in new[] { "OhhO Screen Root", "OhhO Screen", "OhhO Auth Garage Screen", "OhhO Teleop Screen", "OhhO Camera Screen", "OhhO Controls Screen" })
            {
        ...
        private static GameObject BuildCanvas(string name, float width, float height, Vector3 position)
        {
            ...
            AddWindowControls(screen, width, height);
        ...
        private static void AddWindowControls(GameObject screen, float width, float height)
        {
            // 1. Grab Bar (Bottom Center)
            var bar = new GameObject("Grab Bar", typeof(RectTransform));
            bar.transform.SetParent(screen.transform, false);
            var barRt = (RectTransform)bar.transform;
            barRt.anchorMin = new Vector2(0.5f, 0f);
            barRt.anchorMax = new Vector2(0.5f, 0f);
            barRt.pivot = new Vector2(0.5f, 1f);
            barRt.sizeDelta = new Vector2(width * 0.4f, 24f);
            barRt.anchoredPosition = new Vector2(0, -20f);

            var barImg = bar.AddComponent<Image>();
            barImg.color = new Color(1f, 1f, 1f, 0.4f);
            
            var barMani = bar.AddComponent<OmniBot.VR.UI.SpatialWindowManipulator>();
            barMani.targetWindow = (RectTransform)screen.transform;
            barMani.mode = OmniBot.VR.UI.SpatialWindowManipulator.Mode.Move;

            // 2. Resize Handle (Bottom Right)
            var resizeHandle = new GameObject("Resize Handle", typeof(RectTransform));
            resizeHandle.transform.SetParent(screen.transform, false);
            var resRt = (RectTransform)resizeHandle.transform;
            resRt.anchorMin = new Vector2(1f, 0f);
            resRt.anchorMax = new Vector2(1f, 0f);
            resRt.pivot = new Vector2(1f, 1f);
            resRt.sizeDelta = new Vector2(40f, 40f);
            resRt.anchoredPosition = new Vector2(20f, -20f);

            var resImg = resizeHandle.AddComponent<Image>();
            resImg.color = new Color(0f, 0.84f, 1f, 0.6f);
            
            var resMani = resizeHandle.AddComponent<OmniBot.VR.UI.SpatialWindowManipulator>();
            resMani.targetWindow = (RectTransform)screen.transform;
            resMani.mode = OmniBot.VR.UI.SpatialWindowManipulator.Mode.Resize;
        }

        private static (GameObject, GameObject, GameObject, GameObject) BuildScreens()
        {
            var rootGo = new GameObject("OhhO Screen Root");
            rootGo.AddComponent<OmniBot.VR.UI.SmoothHUD>();
            rootGo.transform.position = new Vector3(0, 1.4f, 0);

            float w = 1200f;
            float h = 900f;
            float radius = 1.6f;

            var authScreen = BuildCanvas("OhhO Auth Garage Screen", w, h, Vector3.zero);
            authScreen.transform.SetParent(rootGo.transform, false);
            authScreen.transform.localPosition = new Vector3(0f, 0f, radius);
            authScreen.transform.localRotation = Quaternion.identity;

            var cameraScreen = BuildCanvas("OhhO Camera Screen", w, h, Vector3.zero);
            cameraScreen.transform.SetParent(rootGo.transform, false);
            cameraScreen.transform.localPosition = new Vector3(radius * Mathf.Sin(-45f * Mathf.Deg2Rad), 0f, radius * Mathf.Cos(-45f * Mathf.Deg2Rad));
            cameraScreen.transform.localRotation = Quaternion.Euler(0, -45f, 0);

            var teleopScreen = BuildCanvas("OhhO Teleop Screen", w, h, Vector3.zero);
            teleopScreen.transform.SetParent(rootGo.transform, false);
            teleopScreen.transform.localPosition = new Vector3(radius * Mathf.Sin(45f * Mathf.Deg2Rad), 0f, radius * Mathf.Cos(45f * Mathf.Deg2Rad));
            teleopScreen.transform.localRotation = Quaternion.Euler(0, 45f, 0);

            var controlsScreen = BuildCanvas("OhhO Controls Screen", w, h, Vector3.zero);
            controlsScreen.transform.SetParent(rootGo.transform, false);
            controlsScreen.transform.localPosition = new Vector3(radius * Mathf.Sin(90f * Mathf.Deg2Rad), 0f, radius * Mathf.Cos(90f * Mathf.Deg2Rad));
            controlsScreen.transform.localRotation = Quaternion.Euler(0, 90f, 0);
            ...
            var controlsPage = BuildControlsPage(controlsScreen.transform);
            controlsPage.SetActive(false);

            return (authScreen, teleopScreen, cameraScreen, controlsScreen);
        }
        ...
        private static GameObject BuildControlsPage(Transform parent)
        {
            var page = MakePage("Controls Page", parent);
            var title = MakeText("Title", page.transform, "COMMANDS & ACTIONS", 32, Cyan, TextAlignmentOptions.Center);
            title.fontStyle = FontStyles.Bold;
            Place(title.rectTransform, new Vector2(0.5f, 1f), new Vector2(600, 40), new Vector2(0, -40));

            string helpText = 
                "<b>DRIVING</b>\n" +
                "• Left Joystick: Move Forward/Back/Strafe\n" +
                "• Right Joystick: Turn (Yaw)\n" +
                "• Right Grip: Turbo Speed\n\n" +
                "<b>ROBOT ARM</b>\n" +
                "• Right Hand: Move physical hand to move gripper\n" +
                "• Right Pinch: Open/Close Gripper\n" +
                "• Left Thumbs-Up (Hold 0.5s): Enable/Clutch Arm\n\n" +
                "<b>SAFETY</b>\n" +
                "• Both Grips Squeezed: Emergency Stop\n\n" +
                "<b>DATASET RECORDING</b>\n" +
                "• Start/Stop episodes using the right-side Teleop panel.\n" +
                "• Use the buttons below to trigger ROSBags on the Pi.";

            var text = MakeText("GuideText", page.transform, helpText, 22, TextMain, TextAlignmentOptions.TopLeft);
            text.enableWordWrapping = true;
            Place(text.rectTransform, new Vector2(0.5f, 1f), new Vector2(560, 460), new Vector2(0, -320));

            var rosbagTitle = MakeText("Rosbag Title", page.transform, "ROSBAG RECORDING (ROBOT)", 20, TextMut, TextAlignmentOptions.Center);
            Place(rosbagTitle.rectTransform, new Vector2(0.5f, 0f), new Vector2(600, 30), new Vector2(0, 140));

            var startBag = MakeButton("Start Rosbag", page.transform, "Start Rosbag", 24, Cyan, Color.black);
            Place(startBag.GetComponent<RectTransform>(), new Vector2(0.5f, 0f), new Vector2(240, 60), new Vector2(-130, 60));

            var stopBag = MakeButton("Stop Rosbag", page.transform, "Stop Rosbag", 24, Danger, TextMain);
            Place(stopBag.GetComponent<RectTransform>(), new Vector2(0.5f, 0f), new Vector2(240, 60), new Vector2(130, 60));

            var ctl = page.AddComponent<ControlsPanelController>();
            var so = new SerializedObject(ctl);
            so.FindProperty("startRosbagButton").objectReferenceValue = startBag.GetComponent<Button>();
            so.FindProperty("stopRosbagButton").objectReferenceValue = stopBag.GetComponent<Button>();
            so.ApplyModifiedPropertiesWithoutUndo();
        ...
        // ════════════════════════════════════════════════════════════════════
        private static void WireApp(GameObject authScreen, GameObject teleopScreen, GameObject cameraScreen, GameObject controlsScreen)
        {
            ...
            var controlsPage = controlsScreen.transform.Find("Controls Page")?.gameObject;
            ...
            var ctrlProp = appSo.FindProperty("controlsPanel");
            if (ctrlProp != null) ctrlProp.objectReferenceValue = controlsPage;
        ...
        private static TMP_InputField MakeInputField(string name, Transform parent, string placeholder, float width, float height)
        {
            ...
            field.onSelect.AddListener(_ => TouchScreenKeyboard.Open(field.text, TouchScreenKeyboardType.Default));
            return field;
        }
